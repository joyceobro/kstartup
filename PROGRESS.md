...
- `docs/기능명세서.md` 신규 작성. 실제 코드(`main.py`·`core/*`·`collectors/*`·`frontend/*`) 기준.
- 구성: 개요/설계원칙/스택/환경변수 · 시스템 구성(처리흐름·모듈목록) · 데이터소스 3종 명세 ·
  기능 명세 F-01~F-11(임계값 상수 포함) · API 명세(+200 응답 JSON 스키마) · 화면 명세 ·
  비기능 명세 · 알려진 제약/향후 계획.
- **남은 일**: `<<팀명>>`/`<<팀장 성명>>` 채우기, 제출 형식(PDF/hwpx) 확정, 기획서와 함께 제출.
...
- [~] `docs/기능명세서.md` 초안 완료 → 팀명 채우기·제출 형식 확정만 남음 (위 "기능명세서 초안" 절 참고)
